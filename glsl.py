...
def generate_glsl_shader(weights_path: str, activation: str, threshold: float) -> str:
    ...
    if has_bconv:
        ...
        fn.append("            neigh[idx++] = dot(texture(tex, uv + offset), which_channel);")
        fn.append("        }")
        fn.append("    }")
        fn.append("")
    ...
    if has_bconv:
        ...
        shader.append("            neigh[idx++] = texture(bconv_tex, uv + offset);")
        shader.append("        }")
        shader.append("    }")
    else:
        ...
        shader.append("        for (int dx = -half_k; dx <= half_k; dx++) {")
        shader.append("            vec2 offset = vec2(float(dx), float(dy)) * inv_res;")
        shader.append("            neigh[idx++] = dot(texture(tex, uv + offset), which_channel);")
        shader.append("        }")
        shader.append("    }")
    shader.append("")
