...
def _pack_565(rgb):
    """(n, 3) float32 [0-255] → (n,) uint16 RGB565 with proper rounding."""
    # Scale and round each channel to its target bit depth
    # Red: 5 bits (0-31), Green: 6 bits (0-63), Blue: 5 bits (0-31)
    r = np.round(np.clip(rgb[..., 0], 0, 255) * (31 / 255)).astype(np.uint16)
    g = np.round(np.clip(rgb[..., 1], 0, 255) * (63 / 255)).astype(np.uint16)
    b = np.round(np.clip(rgb[..., 2], 0, 255) * (31 / 255)).astype(np.uint16)
    return (r << 11) | (g << 5) | b
...
def einsum_pc(a, b):
    return np.einsum("npc,npd->ncd", a, b)

def einsum_ij(a, b):
    return np.einsum("nij,nj->ni", a, b)
...
def _pca_endpoints(pixels, iterations=3, gd_iters=3, indexes=4, no_inset=False, emphasize_alpha=0):
    ...
    cov = einsum_pc(centered, centered)          # (n, C, C)
    ...
    def clip_to_ray(e):
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            scales = np.where(d > 0, (255 - mean_color) / d,
                     np.where(d < 0, (0 - mean_color) / d, 1.0))
...
def _pca_endpoints_p(pixels, iterations=3, gd_iters=3, indexes=4, num_threads=3, emphasize_alpha=0):
    ...
    # results is a list of tuples:[(e0_chunk1, e1_chunk1), (e0_chunk2, e1_chunk2), ...]
    # zip(*results) elegantly transposed this to[(e0_chunk1, ...), (e1_chunk1, ...)]
    e0_chunks, e1_chunks = zip(*results)
    
    # Recombine along the batch dimension (axis=0)
    e0_combined = np.concatenate(e0_chunks, axis=0)
    e1_combined = np.concatenate(e1_chunks, axis=0)
    
    return e0_combined, e1_combined

def _endpoints_pca(pixels, iterations=4):
    """PCA endpoint selection for BC1. pixels: (n,16,3) float32 → c0,c1 uint16."""
    e0, e1 = _pca_endpoints_p(pixels, iterations)
...
def _compress_bc1(blocks, pca=0):
    ...
    len_sq = np.maximum(
        ...
        1e-10)
    t = np.clip(
        ((pixels - e1[:, None, :]) * direction[:, None, :]).sum(axis=2) / len_sq,
                                      # (n, 16)
        0, 1)


    # Quantise to 4 palette slots → DXT index, optionally with error diffusion
    if _ERROR_DIFFUSION:
        pix_2d = pixels.reshape(n, 4, 4, 3)                    # (n, 4, 4, 3)
        rgb_err = np.zeros((n, 4, 4, 3), dtype=np.float32)     # accumulated RGB error
        raw_indices = np.empty((n, 4, 4), dtype=np.int32)

        for row in range(4):
            for col in range(4):
                # Shift this pixel by any RGB error diffused into it,
                # then re-project onto the endpoint line to get t.
                adj = pix_2d[:, row, col, :] + rgb_err[:, row, col, :]  # (n, 3)
                t_val = np.clip(
                    ((adj - e1) * direction).sum(axis=1) / len_sq[:, 0],
                    0, 1)                                        # (n,)

                # Quantise in t-space as before.
                q = np.clip(
                    (t_val * 3 + 0.5).astype(np.int32), 0, 3)  # (n,)
                raw_indices[:, row, col] = q

                # Reconstruct the palette RGB for the chosen slot and
                # measure the error in RGB space.
                # In 4-colour mode the palette is e1 + (k/3)*(e0-e1), k=0..3.
                palette_rgb = e1 + (q[:, None] / 3.0) * direction  # (n, 3)
                err_rgb = adj - palette_rgb                      # (n, 3)
                ...
                # Floyd-Steinberg: distribute RGB error to in-block neighbours.
                if col + 1 < 4:
                    rgb_err[:, row,     col + 1] += err_rgb * _wa
                if row + 1 < 4:
                    if col - 1 >= 0:
                        rgb_err[:, row + 1, col - 1] += err_rgb * _wb
                    rgb_err[:, row + 1, col    ] += err_rgb * _wc
                    if col + 1 < 4:
                        rgb_err[:, row + 1, col + 1] += err_rgb * _wd

        indices = _BC1_INDEX_MAP[raw_indices.reshape(n, 16)]    # (n, 16)
    else:
        indices = _BC1_INDEX_MAP[
            np.clip(np.round((t * 3.0)).astype(np.int32), 0, 3)]
...
def _bc7_error_helper(pixels, recon):
    return ((pixels - recon) ** 2)
...
def _bc7_error(pixels, e0, e1, indices, weights):
    ...
    recon = (np.float32(64) - w[:,:,None]) * e0[:,None,:] +  w[:,:,None] * e1[:,None,:]
    recon = np.floor((recon + 32) / 64)
    return _bc7_error_helper(pixels, recon).sum(axis=(1, 2))
...
def _bc7_mode6(pixels, pca=0):
    ...
    def _q6(e):
        ...
        for pb in range(2):
            ...
            err = _bc7_error_helper(e.astype(np.float32), rec.astype(np.float32)).sum(axis=1)
            better = err < best_e
...
def _bc7_mode5(pixels, pca=0):
    ...
    aerr = _bc7_error_helper(alpha, recon_a).sum(axis=1)
    error = cerr + aerr
    ...
    for pid in range(64) if not lite and not nano else [0, 13, 7, 19, 26, 29, 15, 14, 1, 2] if not nano else [0, 13]:
        ...
        for m in (m0, m1):
            ...
            if _PARTITION_PCA:
                ...
                cov  = einsum_pc(cent, cent)            # (n, 3, 3)
                v    = einsum_ij(cov,  v)               # (n, 3)
                norm = np.linalg.norm(v, axis=1, keepdims=True)
...
def _bc7_encode_2subset(pixels, mode, pca=0, best_pid=None, lite=False, nano=False):
    ...
    for pid in range(64):
        ...
        recon = np.floor((np.float32(64) - w[:,:,None]) * base_f +
                          w[:,:,None] * end_f + 32) / 64
        rgb_err = _bc7_error_helper(pix, recon).sum(axis=(1, 2))
        a_err = _bc7_error_helper(pix_full[:,:,3], 255).sum(axis=1)
        error = rgb_err + a_err
            ...
            if has_p:
                def _quant_ep(ev, k=k, cb=cb):
                    ...
                    for pb in range(2):
                        c   = _BC7_QUANT_PB[(cb, pb)][ev_i]
                        dq  = _BC7_DEQUANT_PB[(cb, pb)][c].astype(np.float32)
                        err = _bc7_error_helper(ev, dq).sum(axis=1)
                        bt  = err < best_e
...
def _worker_unpack(args):
    chunk, pca, lite, nano, zero = args
    return _compress_bc7_s(chunk, pca=pca, lite=lite, nano=nano, zero=zero)

def _compress_bc7(blocks_rgba, pca=0, lite=False, nano=False, zero=False, num_threads=8):
    """
    Threaded wrapper for _compress_bc7.
    Parallelizes over the massive batch dimension 'n' (axis=0).
    
    blocks_rgba: (n, 16, 4) uint8
    Returns: (n, 16) uint8 BC7-encoded blocks.
    """
    n = blocks_rgba.shape[0]
    
    # If the array is small enough, skip threading to avoid overhead
    if n <= 2000 or num_threads <= 1:
        return _compress_bc7_s(blocks_rgba, pca=pca, lite=lite, nano=nano, zero=zero)
        
    # Split the massive batch dimension 'n' into equal-sized chunks
    # np.array_split safely handles sizes not perfectly divisible by num_threads
    chunks = np.array_split(blocks_rgba, num_threads, axis=0)
