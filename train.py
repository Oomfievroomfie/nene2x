...
class UpscaleDataset(Dataset):
    ...
    def _add_image(self, hr: torch.Tensor, n_aug: int, brightness_range: float):
        ...
        for _ in range(total_aug):
            ...
            a_for_lr = _orient(hr_for_lr, rot, flip)

            if offset != 0.0:
                a_out    = (a_out    + offset).clamp(0.0, 1.0)
                a_for_lr = (a_for_lr + offset).clamp(0.0, 1.0)

            lr       = manual_downscale2x(a_for_lr)
            baseline = upsample2x(lr)
            residual = a_out - baseline

            self.pairs.append((lr, residual))
...
def train(args: argparse.Namespace):
    ...
    # Model
    model = UpscaleNet(False).to(device)
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=args.epochs, eta_min=args.lr * 0.033
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        # Dummied out intentionally because we don't even have a validation set so Why
        try:
            save_file(model.state_dict(), str(out_path))
        except:
            try:
